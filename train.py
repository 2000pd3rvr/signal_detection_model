from pd3rvr.bbox_regressor import ObjectDetector
from pd3rvr.custom_tensor_dataset import CustomTensorDataset
from pd3rvr import config
...
#initialize images (data), labels, bboxes and image paths as lists
...
#preprocessing
#struct of csv file row: fname, xmin, ymin, xmax, ymax, label
for csvPath in paths.list_files(config.ANNOTS_PATH, validExts=(".csv")):
    ...
    for row in rows:
        row = row.split(",")
        (fidn, xmin, ymin, xmax, ymax, fid) = row

        #get image dimensions from fpath
        imagePath = os.path.sep.join([config.IMAGES_PATH, fid,
        	fidn+'.png'])
        #print('\n\nIMAGE PATH:   {}\n\n'.format(imagePath))
...
print('\n\nData: {}\nLabels: {}\nBBoxes: {}\nImages paths: {}\n\n\n'.format(data, labels, bboxes, imagePaths))
